...
var comp_sym = "o";   //entering and intializing the data
var user_sym = "x";   

onload = function(){
	game = document.getElementById("game");
	arr = game.getElementsByClassName("inner");
	win_block = document.getElementById("win_block");     //shows who is the winner
	win_text = win_block.getElementsByClassName("winner")[0];
	again = win_block.getElementsByClassName("again")[0];  // for the loser or play again on a draft point
	...
	for(var i = 0; i < arr.length; i++){
		arr[i].onclick = function(){
			drawSym(this);          // entering data before array will be full
		};
...
function randomMove(){
	...
	console.log(rnd);
	if (rnd == 1) {  // it will put element randomly in every possible free space
		autoDrawing();
	}
	return true;
}
...
function drawSym(item, sym = user_sym){
	...
	if (winner == user_sym) {
		win_text.innerHTML = "Winner!";       //winners apperance message
		win_text.style.color = "green";
		winning.style.backgroundColor = "rgba(0,200,0, 0.5)";
	}else if (winner == comp_sym) {                      
		win_text.innerHTML = "Loser, try again!";     //losers apperance message
		win_text.style.color = "red";
		winning.style.backgroundColor = "rgba(200,0,0, 0.5)";
	}
	if (winner) {
		winning.style.display = "block";
		win_block.style.display = "block";           //putting or presenting elements in blocks
	}
	return true;
}

function checkWinner(){
	var winner = "";                 // checking who is the winner and check by vertexes and etc
	var j = 0;
	...
	if (!winner){
		for(var i = 0; i < 3; i++){
			...
			if (i != 0) j = 3*i;   //checking by three by column or row
...
function autoDrawing(){
	...
	if (!ckeckFreeSpace()) {
		...
		win_text.innerHTML = "Dead heat!"; // if there is no winner
...
function clearTable(){
	for(var i = 0; i < arr.length; i++){       //start again for again statement 
		arr[i].innerHTML =  "";
	}
}

function ckeckFreeSpace(){     //putting elements into free space
	var res = false;
...
function getRandomInt(max){   //random element shifting put
